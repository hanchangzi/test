...
import gensim
#读入数据
with open('data/semat.pickle', 'rb') as file:#起点终点二分图
    semat = pickle.load(file)
with open('data/smat.pickle', 'rb') as file:#起点时间二分图
    smat = pickle.load(file)
with open('data/emat.pickle', 'rb') as file:#终点时间二分图
...
bigmat[:tarmat.shape[0],tarmat.shape[0]:] = 100*attsmat#4190*attsmat/800
bigmat[tarmat.shape[0]:,:tarmat.shape[0]] = 100*attemat.T#3684*attemat.T/800
...
file1 = open('data/myvec.pickle', 'wb')
pickle.dump(poive, file1)
file1.close()
print('G')
